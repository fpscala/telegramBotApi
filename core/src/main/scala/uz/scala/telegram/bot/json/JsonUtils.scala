...
object JsonUtils {
  ...
  private def underscoreToCamel(name: String) = Predef.augmentString("_([a-z\\d])").r.replaceAllIn(name, { m =>
    m.group(1).toUpperCase()
  })

  private def camelToUnderscores(name: String) = Predef.augmentString("[A-Z\\d]").r.replaceAllIn(name, { m =>
